...
/**
 * Intenta buscar clientes por el término dado.
 * Primero prueba identificación exacta; si falla devuelve [].
 * Cuando la API tenga un endpoint de búsqueda general se puede reemplazar aquí.
 */
async function searchCustomers(term) {
  const clean = term.trim();
  if (!clean) return [];

  // Intentar búsqueda por cédula/RUC (10 o 13 dígitos numéricos)
  ...
  // Para búsqueda por nombre no hay endpoint general aún — devolver vacío
  return [];
}
...
export function CustomerSelector({ value, onChange, error }) {
  ...
  useEffect(() => {
    if (value) return; // already selected — don't re-search
  ...
  const handleInputChange = (e) => {
    // If a customer was selected and user types again, clear selection
    if (value) {
      onChange(null);
      setSelectedName('');
    }
  ...
  const displayValue = value ? selectedName : inputValue;
